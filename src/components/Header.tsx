...
const Header = () => {
  ...
  const socialLinks = [
    ...
    { name: 'Instagram', href: 'https://instagram.com/sivwebai', icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    { name: 'YouTube', href: 'https://youtube.com/@sivwebai', icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
      </svg>
    )},
    { name: 'LinkedIn', href: 'https://linkedin.com/company/sivwebai', icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-center space-x-3">
              {socialLinks.map((social) => (
                <a
                  ...
                  className="w-9 h-9 bg-white/10 hover:bg-accent-lime hover:text-foreground rounded-full flex items-center justify-center text-white transition-all duration-300 hover:scale-110"
                  title={social.name}
                >
                  {social.icon}
                </a>
        ...
        {isMenuOpen && (
          <div className="lg:hidden py-6 border-t border-white/20 bg-foreground/95 backdrop-blur-md">
            <nav className="flex flex-col space-y-2">
              ...
              {/* Mobile Social Icons */}
              <div className="flex items-center space-x-4 py-4 border-t border-white/20 mt-4">
                {socialLinks.map((social) => (
                  <a
                    key={social.name}
                    href={social.href}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 bg-white/10 hover:bg-accent-lime hover:text-foreground rounded-full flex items-center justify-center text-white transition-all duration-300"
                    title={social.name}
                  >
                    {social.icon}
                  </a>
